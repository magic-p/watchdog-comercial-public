window.WATCHDOG_BOOKED_META = {
  "month": "Mar",
  "target": 0,
  "buckets": [],
  "wins": [
    {
      "emoji": "✅",
      "titulo": "Nath Ferreira: Nath: Check entregas de CRM para Aquisição (tarefa anterior: estruturar demanda e conversa com CRM para melhorar visão de closer, baldinhos e ativação",
      "desc": "Concluida no prazo definido.",
      "dri": "Nath Ferreira",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Ana Paula Pimentel: Ana: levantar número de alunos que a escola de Concorrência se propôs a aplicar vs o que foi cadastrado no CRM — identificar delta de upsell perdido",
      "desc": "Concluida no prazo definido.",
      "dri": "Ana Paula Pimentel",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Lili: Lili: revisar orçado x realizado comercial YTD (janeiro)",
      "desc": "Concluida no prazo definido.",
      "dri": "Lili",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Lili: Lili: solicitar a Maria Carol explicação sobre 3 escolas acima de 40k com descarte 'só próximo ano'",
      "desc": "Concluida no prazo definido.",
      "dri": "Lili",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "🎯",
      "titulo": "Lili: Enviar vídeo de renovação (estilo Spotify) para Pimentel e Nath para alinhamento",
      "desc": "Concluida no ciclo semanal (segunda a domingo).",
      "dri": "Lili",
      "data": "13/03",
      "tipo": "ENTREGA_CONCLUIDA"
    },
    {
      "emoji": "🎯",
      "titulo": "Vitoria: Vitoria executar disparo WA para base de Retracao em etapas entrada e descarte para gerar RR — base validada por Nath antes do disparo",
      "desc": "Concluida no ciclo semanal (segunda a domingo).",
      "dri": "Vitoria",
      "data": "13/03",
      "tipo": "ENTREGA_CONCLUIDA"
    },
    {
      "emoji": "✅",
      "titulo": "Vitoria: Nath confirmar execucao e resultado do disparo WA de Retracao em colapso semanal e reportar volume atingido",
      "desc": "Concluida no prazo definido.",
      "dri": "Vitoria",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Nath Ferreira: Nath: analisar 'ponto de partida' (número de alunos do ano anterior) das escolas de Anos Anteriores que converteram — confirmar se houve upsell real ou apenas recontratação no mesmo volume",
      "desc": "Concluida no prazo definido.",
      "dri": "Nath Ferreira",
      "data": "13/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "🎯",
      "titulo": "Nath Ferreira: Nath validar e entregar base elegivel de Retracao e Novas para disparo de MKT — etapas entrada e descarte, excluir SDR ativo",
      "desc": "Concluida no ciclo semanal (segunda a domingo).",
      "dri": "Nath Ferreira",
      "data": "13/03",
      "tipo": "ENTREGA_CONCLUIDA"
    },
    {
      "emoji": "✅",
      "titulo": "Lili: Pimentel criar checklist visual de pontos cruciais para SDRs com base nos insights da WID — insumo para escuta de audio de terca 17/03",
      "desc": "Concluida no prazo definido.",
      "dri": "Lili",
      "data": "12/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: confirmar custos da Revista Estante Mágica com Ericka (produção, design e logística Correios)",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: preparar briefing para freelancers de IG DM e aprovar 5 primeiras DMs antes de escalar",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: escrever copy da cartinha personalizada para mimo chocolate das 200 escolas AA top",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: coordenar envio da 1ª leva de mimo chocolate (100 escolas AA top) via iFood + cartinha",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: enviar report semanal de KPIs do Cerco Mágico para Pedro toda segunda",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Vitoria: Vitória: definir copy do WA utilitário 'SDR vai ligar amanhã' e submeter aprovação Karina",
      "desc": "Concluida no prazo definido.",
      "dri": "Vitoria",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Kaka: Karina: criar plano de ação de marketing para frente LT PART (meta em 10% no dia 09/03)",
      "desc": "Concluida no prazo definido.",
      "dri": "Kaka",
      "data": "10/03",
      "tipo": "ENTREGA_NO_PRAZO"
    },
    {
      "emoji": "✅",
      "titulo": "Ana Paula Pimentel: Pimentel: entregar fotografia real da Renovação reorganizada por faixas de receita até fim da semana",
      "desc": "Concluida no prazo definido.",
      "dri": "Ana Paula Pimentel",
      "data": "09/03",
      "tipo": "ENTREGA_NO_PRAZO"
    }
  ],
  "kpis": {
    "meta_anual_escolas": 3736,
    "meta_mensal_escolas": 0,
    "sdr_meta_pct": 12.0,
    "efetividade_sdr_pct": null,
    "zombie_days": 3,
    "campanha_taxa_verde": 2.0,
    "campanha_taxa_azul": 1.0
  }
};
