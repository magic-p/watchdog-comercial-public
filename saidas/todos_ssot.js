window.WATCHDOG_TODOS = [
  {
    "id": "t179",
    "task": "Calcular e registrar custo por aluno cadastrado por frente de campanha (LT PART vs LT Pública vs HT Pública) — usar como critério de alocação de budget para as semanas restantes de março",
    "comment": "Dados do documento mostram: LT Pública R$0,19/aluno cadastrado, HT Pública R$0,62/aluno cadastrado, LT PART R$2,69/aluno cadastrado. A diferença é uma ordem de grandeza e ainda não está formalizada como critério de decisão de alocação. Insight120 registra o padrão.",
    "owner": "Nath Ferreira",
    "date": "16/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T11:57:00",
    "completedAt": null
  },
  {
    "id": "t180",
    "task": "Pimentel: usar Notebook LM para analisar audios de no-show em Renovacao e identificar padroes de cancelamento/ausencia",
    "comment": "Decisao tomada na daily 13/03. Pedro recusou aceitar o argumento de escola ocupada sem analise mais profunda. 16 mimos nao resolveram o comparecimento (insight125). Objetivo: identificar padrao de quando e por que gestoras agendam mas nao aparecem.",
    "owner": "Ana Paula Pimentel",
    "date": "18/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T00:00:00",
    "completedAt": null
  },
  {
    "id": "t181",
    "task": "Karina e Vitoria: avaliar janela de oportunidade para disparo adicional de Low Touch com acao de video na semana de 16/03",
    "comment": "Decisao pendente de avaliacao. Low Touch em 64% da meta do momento e 23% da meta do mes. Lili sugeriu campanha mais agressiva aproveitando videos. Time de MKT tem plano para responder tickets e atuar como SDR Closer para Low Touch.",
    "owner": "Kaka",
    "date": "16/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T00:00:00",
    "completedAt": null
  },
  {
    "id": "t182",
    "task": "Pedro e Karina: enviar briefing do webinar com Antonia Burk para Pimentel (datas, tema, formato)",
    "comment": "Proximo passo explicito da daily 13/03. Webinar confirmado para a primeira semana de abril. Sem briefing na proxima segunda, Pimentel nao pode iniciar planejamento de divulgacao.",
    "owner": "Pedro",
    "date": "16/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T00:00:00",
    "completedAt": null
  },
  {
    "id": "t183",
    "task": "Vitória: confirmar se disparo WA ampliado para Retração foi executado até 18/03 conforme prazo. Registrar resultado (escolas impactadas, levantadas geradas) na próxima daily. Se não executado, redefinir prazo e remover bloqueio de dados.",
    "comment": "Prazo original era 18/03. Hoje é 18/03. Tarefa precisa ser confirmada como concluída ou ter status atualizado.",
    "owner": "Vitoria",
    "date": "18/mar",
    "tier": "t1",
    "tierLabel": "A fazer",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:04:00",
    "completedAt": null
  },
  {
    "id": "t184",
    "task": "Lili: dar acesso a Vitoria a planilha sendo fechada com Nath — insumo para analise de custo medio de oferta no CPL",
    "comment": "Proximo passo explicito da daily 13/03. Vitoria precisa da planilha para calcular custo medio de vale-presente e sem trabalho e incorporar ao CPL para comparacao de ROI (insight123).",
    "owner": "Lili",
    "date": "13/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T00:00:00",
    "completedAt": null
  },
  {
    "id": "t185",
    "task": "Vitoria: calcular custo medio do vale-presente e do sem trabalho no CPL — apresentar na proxima daily",
    "comment": "Proximo passo da daily 13/03. Depende de t184 (acesso a planilha da Lili). CPL canal + custo medio da oferta por levantada = CPL total. Decisao estrategica: incluir custo da oferta no CPL para comparacao de ROI.",
    "owner": "Vitoria",
    "date": "16/mar",
    "tier": "done",
    "tierLabel": "Concluido",
    "warn": null,
    "done": true,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:04:00",
    "completedAt": "2026-03-18T00:04:00"
  },
  {
    "id": "t186",
    "task": "Karina: confirmar com designers ate 14/03 se pelo menos um aceitou a arte da revista e quando entrega",
    "comment": "Karina confirmou na daily que entraria em contato hoje. Se nenhum designer disponivel, escalar para Pedro imediatamente. Arte da revista e o item com maior lead time do Cerco Magico. Conectada a alerta054.",
    "owner": "Kaka",
    "date": "14/mar",
    "tier": "t1",
    "tierLabel": "A fazer",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-13T00:00:00",
    "completedAt": null
  },
  {
    "id": "t188",
    "task": "Nath: calcular projeção de fechamento de receita via MKT em LT PART com ritmo atual de convertidas/dia (0,56/dia) e apresentar gap esperado até 31/03. Se gap confirmado acima de R$30k, propor segmentação de base por potencial de receita para próximo disparo.",
    "comment": "Alerta065 criado nesta análise. Custo de disparo já em R$731 com apenas 26% da meta de receita de MKT atingida. Ritmo atual é insuficiente para fechar a meta de MKT em LT PART.",
    "owner": "Nath Ferreira",
    "date": "20/mar",
    "tier": "t1",
    "tierLabel": "A fazer",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:04:00",
    "completedAt": null
  },
  {
    "id": "t189",
    "task": "Ana (Pimentel): mapear quantos SDRs estão em ramp-up na frente Novas, qual o tempo médio de rampagem esperado e se há ação planejada de aceleração para as semanas finais de março. Reportar na daily de 20/03.",
    "comment": "Alerta066 criado nesta análise. RR/SDR/Rampando em 55% na semana (0,06/0,11) — subgrupo de rampando é arrasto consistente na frente Novas.",
    "owner": "Ana Paula Pimentel",
    "date": "20/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:04:00",
    "completedAt": null
  },
  {
    "id": "t190",
    "task": "Nath: confirmar se salto de 7 para 16 convertidas em LT PART entre 16/03 e 17/03 é aprovação em lote real ou ajuste contábil retroativo. Registrar causa no próximo update da daily.",
    "comment": "Alerta067 criado nesta análise. A clareza sobre a origem do dado é necessária para calibrar projeção de fechamento de LT PART em março.",
    "owner": "Nath Ferreira",
    "date": "20/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:04:00",
    "completedAt": null
  },
  {
    "id": "t191",
    "task": "Mapear e documentar todos os itens do kit atual da Estante Mágica e criar versão de detalhamento (visual ou textual) para uso no discurso comercial — modelo: Ciranda detalha faixas decorativas, pôsteres, suportes ecológicos, identificadores de mesa e convites",
    "comment": "Decisão explícita da reunião de cliente oculto de 17/03. Deve estar disponível antes dos disparos de vídeo da semana de 23/03 que aumentarão volume de reuniões.",
    "owner": "Bebel Bertuccelli",
    "date": "20/mar",
    "tier": "t1",
    "tierLabel": "A fazer",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:05:00",
    "completedAt": null
  },
  {
    "id": "t191",
    "task": "Convocar alinhamento de discurso comercial com closers e SDRs para incorporar aprendizados do cliente oculto: (1) detalhamento do kit, (2) flexibilidade de formato como solução para carga operacional da escola, (3) SDR humano vs robô Superautor, (4) Magic Land como diferencial frente ao socioemocional da LIV",
    "comment": "Deve ocorrer antes da semana de 23/03. Formato sugerido: 30 minutos, presença de Wydi (closer que fez o estudo) para repassar os achados com contexto.",
    "owner": "Bebel Bertuccelli",
    "date": "20/mar",
    "tier": "t1",
    "tierLabel": "A fazer",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:05:00",
    "completedAt": null
  },
  {
    "id": "t191",
    "task": "Identificar nas escolas em pipeline ativo de renovação (LT PART e HT Particular) quais possuem contrato ou mencionaram LIV como fornecedor de programa socioemocional — criar flag de risco LIV no CRM",
    "comment": "LIV atende mínimo de 60 alunos do Infantil 5 ao 5º ano — filtrar pipeline por esse perfil de escola para priorização.",
    "owner": "Ana Paula Pimentel",
    "date": "25/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:05:00",
    "completedAt": null
  },
  {
    "id": "t191",
    "task": "Criar pelo menos 2 perfis fictícios novos (nome, email, WhatsApp, sem vinculação a contas reais do time) para uso exclusivo nos próximos ciclos de cliente oculto — especialmente para o próximo estudo da LIV com escola fictícia de 400 alunos",
    "comment": "Ciranda identificou e bloqueou participante. Perfis atuais comprometidos para Ciranda. LIV é o próximo concorrente a ser estudado.",
    "owner": "Ana Paula Pimentel",
    "date": "25/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:05:00",
    "completedAt": null
  },
  {
    "id": "t192",
    "task": "Analisar viabilidade de criar mecanismo de urgência temporal (benefício com prazo curto, modelo Supercash da Superautor) para campanhas de fechamento de abril — avaliar formato de benefício incremental por data de assinatura sem alterar preço base",
    "comment": "Superautor usa cashback de 4-10% pago apenas sobre compras até 30 de abril. O mecanismo é de urgência temporal, não desconto permanente — replicável sem impacto estrutural no preço.",
    "owner": "Bebel Bertuccelli",
    "date": "25/mar",
    "tier": "t2",
    "tierLabel": "Fazendo",
    "warn": null,
    "done": false,
    "priorityRank": null,
    "updatedAt": "2026-03-18T00:05:00",
    "completedAt": null
  }
];
